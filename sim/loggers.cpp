...
// CBR Sink Logger
CbrSinkLoggerSampling::CbrSinkLoggerSampling(simtime_picosec period,
                                             EventList& eventlist):
    SinkLoggerSampling(period, eventlist, Logger::CBR_SINK, CbrLogger::RATE)
{
}

void CbrSinkLoggerSampling::monitorSink(DataReceiver* sink) {
    SinkLoggerSampling::monitorSink(sink);  // Call base class to set up _last_seq
    CbrSink *cbrSink = (CbrSink*)sink;
    _last_received.push_back(cbrSink->_received);
}

void CbrSinkLoggerSampling::doNextEvent(){
    eventlist().sourceIsPendingRel(*this,_period);  
    simtime_picosec now = eventlist().now();
    simtime_picosec delta = now - _last_time;
    if (delta == 0) return;  // Skip if no time has passed
    _last_time = now;
    double rate;
    
    // Ensure vectors are properly sized
    while (_last_received.size() < _sinks.size()) {
        _last_received.push_back(0);
    }
    
    for (uint64_t i = 0; i<_sinks.size(); i++){
        if (i >= _last_seq.size()) continue;  // Safety check
        
        CbrSink *sink = (CbrSink*)_sinks[i];
        if (!sink) continue;  // Safety check
        
        if (_last_seq[i] <= sink->cumulative_ack()) {
            // Calculate bytes received (delta)
            uint64_t deltaB = sink->cumulative_ack() - _last_seq[i];
            ...
            // Log: cumulative bytes, packets received, rate
            _logfile->writeRecord(_sink_type, sink->get_id(),
                                  _event_type, sink->cumulative_ack(), 
                                  (double)sink->_received, rate);

            _last_rate[i] = rate;
        }
        _last_seq[i] = sink->cumulative_ack();
        if (i < _last_received.size()) {
            _last_received[i] = sink->_received;
        }
    }
}

string CbrSinkLoggerSampling::event_to_str(RawLogEvent& event) {
    stringstream ss;
    ss << fixed << setprecision(9) << event._time;
    switch(event._type) {
    case Logger::CBR_SINK:
        assert(event._ev == CbrLogger::RATE);
        ss << " Type CBR_SINK ID " << event._id << " Ev RATE"
           << " BytesRcvd " << (uint64_t)event._val1 << " PktsRcvd " << (uint64_t)event._val2 
