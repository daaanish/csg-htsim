...
public:
    ...
    // Set per-tier downlink (egress) link speeds without switching to full custom topology parameters.
    // These speeds are used symmetrically for uplinks as well. Call this BEFORE constructing the topology
    // and then pass linkspeed=0 to the constructor to avoid overriding these values.
    // tor_down: ToR -> host (and host -> ToR via alloc_src_queue)
    // agg_down: Agg -> ToR
    // core_down: Core -> Agg
    static void set_downlink_speeds(linkspeed_bps tor_down, linkspeed_bps agg_down, linkspeed_bps core_down);
