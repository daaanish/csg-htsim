...
for i, snapshot in tqdm.tqdm(enumerate(manifest[start_index:end_index]), total=len(manifest[start_index:end_index])):
    ...
    if mlu.status == GRB.status.OPTIMAL or mlu.status == GRB.OPTIMAL:
        if mlu.ObjVal < 0.01:
            ...
            optimal_sratio = vars_mlu.X
            print(optimal_sratio.shape)
            print("OPTIMAL MLU with TM file:", tm_filename, "is", round(mlu.objVal, 9))
            print("Optimal split ratios for this TM are:", optimal_sratio)
            # look into numpy savetext to save optimal split ratios (they will be saved as 2d matrices)
            # k shortest used by gurobi? paths used by gurobi should match paths used by htsim
            # start working on slides
            # could use gurobi's paths from paths (paths_dict subfolder has all the paths) folder
            # Save optimal split ratios for this TM
            split_ratio_filename = f"{results_path}/{num_cluster}/split_ratios_snapshot_{index}.npy"
            np.save(split_ratio_filename, optimal_sratio)
