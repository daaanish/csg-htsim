...
struct connection{
    ...
    int paths; // number of ECMP paths to use (>=2 enables multipath)
    std::vector<double> split_ratios; // optional split ratios per path (normalized)
    std::vector<int> path_indices; // optional explicit ECMP path indices (0-based)
    // Threshold-based path switching: switch to path i+1 after sending byte_thresholds[i] bytes
    std::vector<uint64_t> byte_thresholds;
