...
class CbrSinkLoggerSampling : public SinkLoggerSampling {
    virtual void doNextEvent();
 public:
    CbrSinkLoggerSampling(simtime_picosec period, EventList& eventlist);
    void monitorSink(DataReceiver* sink);  // Override to initialize _last_received
    static string event_to_str(RawLogEvent& event);
private:
    vector<uint32_t> _last_received;  // Track packets received
};
