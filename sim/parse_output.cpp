...
int main(int argc, char** argv){
    if (argc < 2){
        printf("Usage %s filename [-show|-verbose|-ascii|-names]\n", argv[0]);
        return 1;
    }

    bool show = false, verbose = false, ascii = false, dump_names = false;
    ...
    while (i<argc) {
        if (!strcmp(argv[i],"-idmap")){
            ...
            ascii = true;
        } else if (!strcmp(argv[i],"-names") || !strcmp(argv[i],"--names") || !strcmp(argv[i],"-dump-names")){
            dump_names = true;
    ...
    */
    ...
    // If requested, dump ID -> name mapping and exit
    if (dump_names) {
        // copy keys to a vector and sort for stable output
        vector<int> keys; keys.reserve(object_names.size());
        for (auto &kv : object_names) keys.push_back(kv.first);
        sort(keys.begin(), keys.end());
        for (int id : keys) {
            const string &nm = object_names[id];
            // Use tab separator so names with spaces are preserved
            printf("%d\t%s\n", id, nm.c_str());
        }
        return 0;
    }
    ...
    if (argc>2&&!strcmp(argv[2], "-memory")) {
        ...
        TYPE = 18; EV = 1800;
    } else if (argc>2&&!strcmp(argv[2], "-cbr")) {
        TYPE = 45; EV = 4500;
    ...
    for (int i=0;i<numRecords;i++){
        ...
        if (ascii) {
            ...
            case Logger::NDP_SINK: //18
                ...
                break;
            case Logger::CBR_SINK: //45
                out = CbrSinkLoggerSampling::event_to_str(event);
                break;
